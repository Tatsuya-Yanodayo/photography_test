...
class MLAShortcode_Support {
	...
	public static function mla_gallery_shortcode( $attr, $content = NULL ) {
		...
		foreach ( $attachments as $id => $attachment ) {
			...
			$mlaTag = get_field('portfolio_tag', $attachment->ID); // 値の代入
			$item_values['mla_tag'] = implode(' ', $mlaTag); // 配列要素を文字列「, 」で連結して表示
			//portfolio.phpにて使用のため追加しました！
			...
			switch ( $arguments['link'] ) {  //linkpoint
//				case 'permalink':
				case 'post':
					$item_values['link'] = $item_values['pagelink'];
					break;
				case 'permalink':  //勝手に移動しました
				case 'file':
				case 'full':
